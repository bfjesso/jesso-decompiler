...
FunctionsWindow::FunctionsWindow(wxWindow* parent, const wxSize& size, ColorsMenu* colorMenu, wxStaticText* statusText) : JdcTextCtrl(parent, size, statusText)
{
	colorsMenu = colorMenu;
	EnableLineNumbers();
}

void FunctionsWindow::ShowAllFunctions(struct DecompilationParameters* params)
{
	ClearText();
	SetReadOnly(false);
	Freeze();
...
void FunctionsWindow::ApplyFunctionsHighlighting()
{
